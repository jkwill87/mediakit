...
use mediakit::meta::fields::{MediaFormat, TrackDisposition, TrackKind};

#[test]
fn structured_external_track_metadata_is_public() {
    let inspector = FilenameInspector::new("Rango.2011.pt-BR.2.forced.srt").analyze();

    assert_eq!(inspector.metadata.format, Some(MediaFormat::Srt));
    assert_eq!(inspector.metadata.identity_stem(), Some("Rango.2011"));
    assert!(!inspector.metadata.has_generic_identity());

    let track = inspector.metadata.track.expect("external track metadata");
...
fn ordinary_media_uses_the_same_format_model_without_track_metadata() {
    ...
    assert_eq!(inspector.metadata.format, Some(MediaFormat::Mkv));
    assert_eq!(inspector.metadata.track, None);
    assert_eq!(inspector.metadata.identity_stem(), Some("Rango.2011"));
}
...
fn non_utf8_external_tracks_keep_structured_format_metadata() {
    ...
    assert_eq!(inspector.metadata.format, Some(MediaFormat::Srt));
    assert!(inspector.metadata.track.is_some());
    assert_eq!(inspector.metadata.identity_stem(), None);
}
